...
账号维护的跨日游标及每日完成状态已由同日修订《猫眼账号维护扫描简化设计》取代；本文件的其他监控和通知通道设计继续适用。
...
| 账号维护 | 默认 `[01:00, 02:00)`，管理员可改 | 每个维护 tick 重查未处理候选，分页产生提醒并处理三十天归档和撤销清理；关窗后仅核验 |
...
维护分别为提醒、三十天归档和撤销清理保存 `(job, localDate)` 完成状态与短租约，不再使用持久游标续跑。每次 cron 在配置窗口内按当前有效候选处理有界页，先处理当日提醒；同日新候选会在后续 tick 继续进入查询。窗口结束后只核验当日是否仍有候选，不生成或发送旧阶段提醒；归档与撤销清理在次日重新从未处理业务标记继续。持续失败占满一页时记录积压告警，提醒过了有效自然日不补发。三十天归档和撤销清理仍受原有业务版本及条件保护。若 cron 被停用或配置到完全错过维护窗口，无法承诺自然日前提醒；看板区分未开始观测、未执行、未完成和已完成，外部健康检查可据此报警。同一条已停用的 cron 无法自行发现并报告自己的停用。
