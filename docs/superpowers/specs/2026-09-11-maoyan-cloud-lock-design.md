...
监控与未来锁座共用同一个可调整 cron，不再配置独立锁座 cron。Cloudflare Dashboard 或 `wrangler.toml` 修改监控 cron 后，两项能力同时按新频率运行。`scheduled` 事件先完成监控抓取、快照和状态持久化，再把本次已确认的影院场次数据交给锁座协调器；监控失败、停止或到期时，本轮不执行锁座。

目标日期的真实场次在用户提交规则时已经存在，则沿用即时锁座路径，不等待 cron。只有目标场次尚未出现的规则才进入 `waiting_schedule`；此后它不得自行查询排期，只能消费成功监控得到的场次数据。

对每个活动规则：

1. 接收本次监控成功并完成持久化的实际排期。
...
- 使用结构化对象记录 `scope`、`event`、`phase`、`state`、`seatCount`、`httpStatus` 和有限的错误分类，便于 Cloudflare Query Builder 按字段查询。
- 日志不得包含访问 token 或 token ID、UID、影院/影片名称、日期时间、场次号、座位号、订单号、Cookie、`mtgsig`、动态查询参数、猫眼原始响应、内部 URL 或响应请求头。
- 无活动规则时不输出逐 token 日志；Cloudflare invocation log 足以证明 cron 被触发。
- 历史日志按 Cloudflare 保留策略自然过期，不复制或重新输出旧的敏感内容。
...
4. 只有第 3 步成功，才启用 `LOCK_SERVICE_ENABLED`；未来规则随监控 cron 执行，不新增锁座 cron。
5. 验证严格顺序为“监控成功并持久化 -> 锁座匹配”，且监控失败、停止或到期时不锁座。
6. 验证严格时间匹配、座位变化失败、目标日期过期、会话删除和不确定订单结果不会重复提交。
