...
- Electron 安装包由 GitHub Actions 发布到 GitHub Releases；下载页与客户端更新检查均使用 GitHub，不依赖额外镜像或对象存储

### Web 与桌面端维护

- 两端共用 `pages/maoyan` 的监控、选座和通知业务；`runtime.js` 统一管理平台能力，页面通过 `capabilities` 控制下载、更新和工具箱入口，不复制业务页面
- Web 展示客户端下载入口；Electron 登录页和顶栏展示应用版本与检查更新入口，不展示客户端下载和工具箱入口
- Electron 领取密钥使用隔离的 HTTPS 窗口，无 Node 权限或特权 preload；隐藏已有密钥输入、下载和进入 Web 按钮，领取后复制密钥回主窗口使用。旧客户端缺少窗口接口时回退系统浏览器
- 客户端更新是提示后手动下载安装，不是自动安装；自动检查每天最多一次，手动检查不受每日缓存限制，网络失败与“已是最新版本”明确区分
- 共享页面变更需同时运行页面单测、Electron smoke 和浏览器跨视口测试；Web 部署不能更新已安装客户端的本地页面，共享页面或桌面桥接变化需发布新客户端
- 桌面开发、打包和安全边界详见 [desktop/README.md](desktop/README.md)
