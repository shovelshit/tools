// ---------------- cron 批次解析 ----------------
// 定时批次在 wrangler.toml [triggers].crons 与下面的 CRON_EXPRESSION 定义。
// 管理员业务窗口由 D1 策略控制；cron 始终每三分钟触发一次。
  ...
  try {
    if (env && env.CF_API_TOKEN && env.CF_ACCOUNT_ID) {
      ...
      const list = data && data.success && Array.isArray(data.result)
        ? data.result.map((s) => s && s.cron).filter((cron) => cron && String(cron).trim())
        : [];
