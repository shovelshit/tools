...
export function accountPrincipal(account, credentialType, nowMs) {
  return {
    userId: account.id,
    role: account.role,
...
export async function ensureAdminAccount(env, nowMs = Date.now()) {
...
export async function authenticate(request, env, nowMs = Date.now()) {
  ...
  if (account) {
    ...
    return accountPrincipal(account, "access_key", nowMs);
  }
  ...
  return accountPrincipal(monitorAccount, "admin_monitor_session", nowMs);
}
...
export async function exchangeSession(request, env, nowMs = Date.now(), businessLine = "maoyan") {
  const raw = String(request.headers.get("X-Token") || "").trim();
  ...
  if (configuredAdmin && raw === configuredAdmin) {
    const account = await ensureAdminAccount(env, nowMs);
